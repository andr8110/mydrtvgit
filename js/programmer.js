...
function sortByGenre() {
    ...
    $.ajax({
        ...
        if (jData.status !== 0) {
            ...
            for (x in jData) {
                // console.log(jData[x]);
                var resultDiv = `<li data-name='${jData[x].title}' data-genre='${jData[x].name}' class='list-inline-item archiveItem col-xs-12 col-sm-6 col-md-3 col-lg-3'><a id='${x}' href='videoplayer.php?movieID=${jData[x].id}'><span class='col-xs-12 col-sm-12 col-md-12 col-lg-12 pull-left programListColFix programTitle'>${jData[x].title}</span></a></li>`
                $('#archiveContainer').append(resultDiv)
            }
        }
    })
